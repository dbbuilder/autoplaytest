...
Uses Anthropic's Claude API for test generation
"""
...
class ClaudeProvider(BaseAIProvider):
    """Claude AI provider for test generation"""
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        api_key = os.getenv(self.config.get('provider', {}).get('api_key_env', 'ANTHROPIC_API_KEY'))
        if not api_key:
            raise ValueError("Claude API key not found in environment variables")
        
        # Initialize Claude client
        self.client = AsyncAnthropic(api_key=api_key)
        self.model = self.config.get('models', {}).get('default', 'claude-3-opus-20240229')
    ...
    async def validate_test(self, test_code: str) -> Tuple[bool, List[str]]:
        """Validate generated test code using Claude"""
        ...
        message = await self.client.messages.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": validation_prompt.format(code=test_code)
                }
            ],
            max_tokens=1000,
            temperature=0.1
